        ...
        <div className="overflow-x-auto no-scrollbar">
          <div className="flex gap-6 items-start py-2 px-2">
            {categories.map((cat, idx) => {
              ...
              const onClick = () => {
                window.history.pushState({}, '', `/projects/${encodeURIComponent(cat.key)}`);
                ...
                >
                  ...
                  <div className="relative h-56 bg-black overflow-hidden rounded-xl">
                    {isImage ? (
                      ...
                      cat.items && cat.items.length > 0 && cat.items[0].source === 'youtube' ? (
                        <img src={`https://img.youtube.com/vi/${cat.items[0].videoUrl}/maxresdefault.jpg`} alt={cat.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                      ) : (
